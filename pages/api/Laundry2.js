...
export default async function AuthAPI (req, res) {
    ...
    const [Laundry] = await db.select('*').from('Laundrys').where('num', 2)
    if (req.method === 'POST') {
        if (Laundry.realname == checktoken.realname && Laundry.room == checktoken.room && Laundry.use == 1){
            await db('Laundrys').update({ use: 0, room: 0, realname: 0 }).where('num', 2)
            return res.send({ success : true })
        }else if (Laundry.realname == 0 && Laundry.room == 0 && Laundry.use == 0){
            await db('Laundrys').update({ use: 1, room: checktoken.room, realname: checktoken.realname }).where('num', 2)
            return res.send({ success : true })
